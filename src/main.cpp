...
int64_t GetBlockValue(int nHeight)
{
 ...
 if (Params().NetworkID() == CBaseChainParams::TESTNET) {
    ...
    // Ancient Knolwedge Phase 2 proposal
    else if (nHeight <= 224999 && nHeight >= 150000) {
        nSubsidy = 150 * COIN;
    }
    else if (nHeight <= 299999 && nHeight >= 225000) {
        nSubsidy = 125 * COIN;
    }
    else if (nHeight <= 349999 && nHeight >= 300000) {
        nSubsidy = 110 * COIN;
    }
       else if (nHeight <= 399999 && nHeight >= 350000) {
        nSubsidy = 105 * COIN;
    } 
        else if (nHeight <= 449999 && nHeight >= 400000) {
        nSubsidy = 100 * COIN;
    }
        else if (nHeight <= 951470 && nHeight >= 450000) {
        nSubsidy = 85 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDKPCStake)
{
int64_t ret = blockValue * 0.9;
if (nHeight < 149999) {
        ret = blockValue * 0.9;
    }
        ...
        //When zDKPC is staked, masternode only gets 0.70
              if (isZDKPCStake)
         ret = blockValue * 0.70;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateDKPCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zDKPC money supply in the block index
     if (!UpdateZPIVSupply(block, pindex, fJustCheck)) 
        return state.DoS(100, error("%s: Failed to calculate new zDKPC supply for block=%s height=%d", __func__,
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zDKPC stake block is less than allowed start height", __func__);

    if (CZDkpcStake* zDKPC = dynamic_cast<CZDkpcStake*>(stake)) {
        CBlockIndex* pindexFrom = zDKPC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zDKPC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zDKPC stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zDKPC->GetValue()));
        if (nChecksum200 != zDKPC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zDKPC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZDKPC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zDKPC fails context checks", __func__));
