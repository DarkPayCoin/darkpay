...
*Setup instructions for a gitian build of DARKPAYCOIN using a Debian VM or physical system.*
...
- [Building DARKPAYCOIN](#building-darkpaycoin)
...
Clone the git repositories for darkpaycoin and gitian and then checkout the darkpaycoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/darkpaycoin-crypto/darkpaycoin.git
cd darkpaycoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the darkpaycoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building DARKPAYCOIN
----------------

To build DARKPAYCOIN (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the darkpaycoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/darkpaycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/darkpaycoin-crypto/darkpaycoin
...
For example:
```bash
URL=https://github.com/crowning-/darkpaycoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/darkpaycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/darkpaycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/darkpaycoin-build.assert
```
...
[darkpaycoin/gitian.sigs](https://github.com/darkpaycoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
